...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_APPS_MONOREPO_xxx
namespace = Collection("apps_monorepo")
namespace.configure(
    {
        "apps_monorepo": {
            "nautobot_ver": "2.3.1",
            "project_name": "apps-monorepo",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.apps_monorepo.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.apps_monorepo.compose_http_timeout,
        "NAUTOBOT_VER": context.apps_monorepo.nautobot_ver,
        "PYTHON_VER": context.apps_monorepo.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.apps_monorepo.project_name}",
        f'--project-directory "{context.apps_monorepo.compose_dir}"',
    ]

    for compose_file in context.apps_monorepo.compose_files:
        compose_file_path = os.path.join(context.apps_monorepo.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.apps_monorepo.local):
...
# TODO: why are these apps failing to build?
...
def _get_all_app_directories():
    ...
    for dir in parent_dir.iterdir():
        ...
        if dir.is_dir() and tomlfile.is_file():
            try:
                with open(tomlfile, "rb") as fp:
                    app_config = tomllib.load(fp)
                app_name = app_config.get("tool", {}).get("poetry", {}).get("name", None)
                if app_name and app_name.startswith("nautobot-") and app_name not in IGNORE_APPS:
                    if dir.name in app_dirs.values():
                        raise ValueError(f"Duplicate app directory found: {dir}")
                    app_dirs[app_name] = dir
    ...
    for app_name, app_dir in app_dirs.items():
        copied_app = apps_copy_dir / Path(app_dir).name
...
def _generate_dockerfile(app_dirs):
    ...
    dockerfile_output = Path(os.path.dirname(__file__)).resolve() / "development" / "Dockerfile_generated"
    with open(dockerfile_output, "w") as f:
...
def generate_docker_override_file(context):
    ...
    dockerfile_output = Path(os.path.dirname(__file__)).resolve() / "development" / "docker-compose.override.yml"
    with open(dockerfile_output, "w") as f:
        f.write(template.render(app_dirs=app_dirs))
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.apps_monorepo.python_ver
    dockerfile_path = os.path.join(context.apps_monorepo.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.apps_monorepo.python_ver}"
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations apps_monorepo"
...
@task(
    help={
        "version": "Version of Apps Monorepo to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml apps_monorepo'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'apps_monorepo/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.apps_monorepo.local):
